...
if __name__ == "__main__":
	env = os.environ.copy()
	cmd=['cpplint','--filter=-readability/utf8,-whitespace/ending_newline,-readability/casting,-runtime/references,-whitespace/newline','--linelength=120']
	
	for item in sys.argv[1:]:
		cmd.append(item)
	ret = subprocess.call(cmd, stdout=sys.stdout, env=env)
    # abspath is to avoid requiring './' on Linux
	#print("ret %d" %(ret));
